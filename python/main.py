...
class MainForm(QtWidgets.QMainWindow, main_window.Ui_MainWindow):
    ...
    def update_image(self):
        ...
        h=240
        w=320
        d=1   # 2*uint8 :2byte:16bitRGB

        if buf[0:1]!=b'\xff': return
        if buf[-8:]!=b'\x00\x00\x00\x00\x00\x00\x00\x00': return
        print(len(buf[1:-8]))
        # assert(len(buf[1:-8])==h*w)
        # self.image = np.fromstring(buf[1:-8], dtype=np.uint8)
        self.qimage = QtGui.QImage(buf[1:-8], w, h, w*d, QtGui.QImage.Format_Grayscale8)
...
class Reciever(QtCore.QThread):
    ...
    def run(self):
        ...
        # w=320
        # d=2   # 2*uint8 :2byte:16bitRGB
        # self.image = np.zeros((h,w,d), dtype='uint8')
        #
        # for i in range(h):
        #     for j in range(w):
        #         r = int(i*0b1111/h)
        #         g = int(j*0b1111/w)
        #         b = int(0)
        #         self.image[i,j,0] = g << 4 | b
        #         self.image[i,j,1] = 0 << 4 | r
        #
        # self.mutex.lock()
        # self.buf_list.append(b''.join([b'\xff',self.image.tobytes(),b'\x00\x00\x00\x00\x00\x00\x00\x00']))
